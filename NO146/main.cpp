...
// 双向链表存储缓存内容，用双向链表是删除、插入在O(1)操作下就能够完成
// 使用unordered_map存储key和节点，使查找可以在O(1)操作下完成
class LRUCache {
...
public:
    ...
    void put(int key, int value) {
        if(!cache.count(key)) {
            // 如果map中不存在， 则创建节点，插入到头部，然后判断是否需要size是否超过capacity
            ...
            addToHead(node);
            size++;
            // 大小超过容量，删除最后一个节点，并从map中删除对应的key node
            if(size > capacity) {
